...
const ProjectDetail = () => {
    ...
    useEffect(() => {
        setLoading(true);
        fetch(`https://ghsoyad-portfolio-server.vercel.app/project/${projectName}`)
            .then(res => res.json())
            .then(data => {
                setLoading(false);
    ...
    return (
        <section id='projects' className='container mx-auto max-w-screen-xl px-4 xl:px-0 py-8'>
            <div className='divider w-full md:w-1/2 mx-auto before:bg-primary after:bg-primary before:h-0.5'>
                <h3 className='font-bold text-2xl md:text-4xl text-center pb-2'>{projectName.split('-').join(' ')}</h3>
            </div>
            {loading ? <Loader>Loading Project Details...</Loader> :
                            ...
                            <div className='grid grid-cols-2 gap-4 max-h-32'>
                                {
                                    images.map((image, i) =>
                                        <PhotoView key={i} src={image}>
                                            <img src={image} alt="" className='w-full max-h-32 h-full object-cover object-top rounded-md shadow hover:shadow-lg cursor-pointer' />
                                        </PhotoView>
                                    )
                                }
                            </div>
